...
import { singleProject, updateProject } from '../../actions/project';
const ReactQuill = dynamic(() => import('react-quill'), { ssr: false });
import '../../node_modules/react-quill/dist/quill.snow.css';
import { QuillModules, QuillFormats } from '../../helpers/quill';
import { API } from '../../config';

const ProjectUpdate = ({ router }) => {
    ...
    const initProject = () => {
        if (router.query.slug) {
            singleProject(router.query.slug).then(data => {
                if (data.error) {
    ...
    const editProject = e => {
        e.preventDefault();
        updateProject(formData, token, router.query.slug).then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error });
            } else {
                setValues({ ...values, title: '', success: `Project titled "${data.title}" is successfully updated` });
