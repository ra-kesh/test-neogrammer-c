...
const CreateProject = ({ router }) => {
    ...
    const [body, setBody] = useState(projectFromLS());
    ...
    const publishProject = e => {
        ...
        createProject(formData, token).then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error });
            } else {
                setValues({ ...values, title: '', error: '', success: `A new Project titled "${data.title}" is created` });
                setBody('');
                setCategories([]);
                setTags([]);
            }
    ...
    const handleChange = item => e => {
        ...
        const value = item === 'photo' ? e.target.files[0] : e.target.value;
        formData.set(item, value);
        setValues({ ...values, [item]: value, formData, error: '' });
    };
